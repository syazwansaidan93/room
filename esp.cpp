...
void handleData() {
    ...
    jsonDoc["night_led_brightness_level"] = nightLedPercent;
    ...
    jsonDoc["current_time"] = timeString;
...
void loop() {
    ...
    // Proximity switch logic for master switch
    ...
    // Time-based night LED control
    struct tm timeinfo;
    if(getLocalTime(&timeinfo)){
        int currentHour = timeinfo.tm_hour;
        int currentMinute = timeinfo.tm_min;

        bool nightTimeActive = false;
        ...
        // Night LED logic: on if it's night time AND (main LED is off OR master switch is off and main LED is on)
        if (nightTimeActive && (!mainLedManualState || (!proximityManualState && mainLedManualState))) {
            ledc_set_duty(ledcMode, ledcChannel_ldr, currentBrightnessDutyCycle);
        } else {
    ...
    // Main LED and Fan control logic, dependent on the master switch
    if (proximityManualState) {
        // Main LED manual state toggle logic, now dependent on master switch
        ...
        if (currentMode == AUTOMATED) {
            bool offConditionMet = (lastTemperature <= tempOff);
            bool onConditionMet = (lastTemperature >= tempOn);

            if (digitalRead(fanrelay) == LOW) { // Fan is currently off, check if it should turn on
                if (onConditionMet) {
                    digitalWrite(fanrelay, HIGH);
                }
            } else { // Fan is currently on, check if it should turn off
                if (offConditionMet) {
                    digitalWrite(fanrelay, LOW);
                }
            }
        ...
        // If master switch is off, turn off main LED and fan.
